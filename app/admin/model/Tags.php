...
class Tags extends Model
{
    ...
    {
//        if (!is_numeric($tag['tid']) && empty($tag['arcid'])) {
//            return [];
//        }
        ...
        if (!env('APP_DEBUG') && $cacheData = cache($cacheID) && $tag['page']!=1) {
            return $cacheData;
        }
        ...
        $lang = site('content_lang_on')==1?app()->lang->getLangset():site('content_lang');
        $map = [['lang','=', $lang]];
        if ($tag['tid']) {
            $map[] = ['id','=', $tag['tid']];
        }
    ...
    public function getContent($tag, &$page)
    {
        ...
        // 结果进行缓存
        if (!env('APP_DEBUG')) {
            cache($cacheID, $newArray, $cacheTime);
        }
        return $newArray;
    }
