...
use addons\user\model\User as UserModel;
...
class User extends Auth
{
    ...
    public function login(string $username, string $password) : bool
    {
        ...
        // 设置token登录信息
        $info->token = md5(Str::random(10).$info->id.$info->login_ip);
        app('token')->set($info->token, $info->id);

        $this->user = $info;

        event('user_login_success', $info);
        return true;
    }
    ...
    public function loginOut()
    {
        ...
        app('token')->delete($this->user->token);
        event('user_logout_success', $this->user);
        $this->user = null;
        return true;
    }
