...
use addons\user\middleware\Login;
...
//        Auth::class
    ...
    public function initialize()
    {
        ...
        // 登录事件
        Event::listen('user_login_success', function($user) {
            $auto_signin = input('post.auto_signin','');
            $expire = empty($auto_signin) ? null : 2592000; // 空，使用默认的，不为空30天
            Cookie::set('utoken', $user['token'], $expire);
        });
        // 退出登录事件
        Event::listen('user_logout_success', function($user) {
            Cookie::delete('utoken');
        });

        $this->userConfig = get_addons_config('addon','user');
        $this->view->assign('config', $this->userConfig);

        $this->user = \addons\user\library\User::instance();
    ...
    public function profile()
    {
        if ($this->request->isPost()) {
            ...
            if (\addons\user\model\User::where(['nickname'=>$row['nickname']])->where('id','<>', $u->id)->find()) {
    ...
    public function bind()
    {
        if ($this->request->isPost()) {
            ...
            if (!empty($row['mobile'])) {
                ...
                $info = \addons\user\model\User::where(['mobile'=>$row['mobile']])->find();
                if ($info) {
                ...
                \addons\user\model\User::where('id', $this->user->id)->save(['mobile'=>$row['mobile']]);
                ...
                $info = \addons\user\model\User::where(['email'=>$row['email']])->find();
                if ($info) {
                ...
                \addons\user\model\User::where('id', $this->user->id)->save(['email'=>$row['email']]);
                $this->success(lang('Success'));
            } else {
    ...
    public function login()
    {
        if ($this->request->isPost()) {
            ...
            // 开启验证码
            if ($this->userConfig['login_captcha']==1) {
                if (empty($row['code'])) {
        ...
        $token = $this->request->server('authorization', $this->request->param('utoken', Cookie::get('utoken','')));
        $bl = \addons\user\library\User::instance()->checkToken($token);
        if ($bl) {
    ...
    public function register()
    {
        if ($this->request->isPost()) {
            ...
            // 判断用户名是否已存在
            if (\addons\user\model\User::getByUsername($row['username'])) {
                $this->error(lang('Username already exists'),'',[],0,['__token__'=>$this->request->buildToken()]);
            }
            // 验证码验证
            if ($this->userConfig['register_captcha_type']==2) { // 邮箱
                if (site('mail_on')!=1) {
                ...
                if (\addons\user\model\User::getByEmail($row['email'])) {
            ...
            } else if ($this->userConfig['register_captcha_type']==3) { // 手机
                if (empty($row['mobile'])) {
                    $this->error(lang('Please fill in your phone number'),'',[],0,['__token__'=>$this->request->buildToken()]);
                }
                if (\addons\user\model\User::getByMobile($row['mobile'])) {
        ...
        if ($this->request->isPost()) {
            ...
            if ($status==1) {
                ...
                if (Validate::is($username, 'email')) {
                    $info = \addons\user\model\User::where(['email'=>$username])->find();
                    if (!$info) {
                    ...
                    $info = \addons\user\model\User::where(['mobile'=>$username])->find();
                    if (!$info) {
                ...
                if ($type==2) { // 邮箱
                    ...
                    $info = \addons\user\model\User::where(['email'=>$mark])->find();
                    if (!$info) {
                        $this->error(lang('Email does not exist'));
                    }
                    ...
                    $info = \addons\user\model\User::where(['mobile'=>$mark])->find();
                    if (!$info) {
                        $this->error(lang('Phone number does not exist'));
                    }
